...
function render() {
  ...
  if (!lastValidGrammar) {
    shadowRoot.getElementById('output').textContent = '⚠ Fix grammar errors to see output.';
    return;
  }
  try {
    const grammar = createGrammar(lastValidGrammar);
    const result = grammar.generate('origin');
    ...
    shadowRoot.getElementById('output').textContent = `Runtime error: ${e.message}`;
  }
}
...
function getSharableCompiledState() {
  ...
  try {
    ...
    if (!compiled?.symbols?.origin) {
      return { ok: false };
    }

    compiled.generate('origin');
...
async function syncUrlToCurrentState() {
  ...
  const url = await buildShareURL(state.grammar);
